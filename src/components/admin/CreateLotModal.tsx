    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="bg-white border border-slate-200 rounded-[2.5rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl animate-slide-up flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="sticky top-0 z-10 bg-white/90 backdrop-blur px-8 py-6 border-b border-slate-150 flex items-center justify-between">
          ...
          <button onClick={onClose} className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-all shadow-sm cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className={labelCls}><Hash className="w-3 h-3 inline mr-1 text-slate-400" />Número de Lote *</label>
              <input value={form.number} onChange={e => set("number", e.target.value)} placeholder="Ej: L-60" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}><Layers className="w-3 h-3 inline mr-1 text-slate-400" />Etapa</label>
              <input value={form.stage} onChange={e => set("stage", e.target.value)} placeholder="Opcional" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}><MapPin className="w-3 h-3 inline mr-1 text-slate-400" />Área m²</label>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={onClose} className="flex-1 px-4 py-3.5 rounded-xl border border-slate-200 text-[10px] font-bold uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-colors cursor-pointer">
              Cancelar
            </button>
            <button
