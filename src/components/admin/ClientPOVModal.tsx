              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm text-slate-850 font-bold focus:border-blue-600 focus:ring-2 focus:ring-blue-600/10 focus:outline-none cursor-pointer appearance-none animate-none"
                    style={{
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3">
              ...
              {selectedObjects.filter((c: any) => c.number > 0).reduce((acc: number, c: any) => acc + (c.penaltyAmount || 0), 0) > 0 && (
                <div className="flex justify-between text-xs font-medium text-slate-500">
                  ...
                  <span className="font-bold text-red-655 font-bold">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                    {selectedObjects.filter((c: any) => c.hasPenalty || c.penaltyAmount > 0).map((cuota: any, idx: number) => {
                      if (cuota.isHistorical || cuota.number === 0) {
                        return (
                          <tr key={idx} className="hover:bg-slate-50/30 font-medium">
                            ...
                            <td className="px-4 py-3 text-slate-405 italic">Previa</td>
                            <td className="px-4 py-3 text-slate-405 italic">—</td>
                            <td className="px-4 py-3 text-slate-450 text-right">—</td>
                            <td className="px-4 py-3 font-bold text-red-655 text-right">{formatCLP(cuota.penaltyAmount || cuota.amount)}</td>
                          </tr>
                        );
                      }
                      return (
                        <tr key={idx} className="hover:bg-slate-50/30 font-medium">
                          ...
                          <td className="px-4 py-3 text-slate-500 text-right">{formatCLP(cuota.dailyPenalty || data.dailyPenalty || 1500)}</td>
                          <td className="px-4 py-3 font-bold text-red-655 text-right">{formatCLP(cuota.penaltyAmount)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
