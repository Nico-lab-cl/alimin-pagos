    ...
    >
      ...
      >
        ...
        <div className="sticky top-0 z-20 bg-white border-b border-slate-200 shadow-sm shrink-0">
          {/* POV Warning Banner */}
          <div className="bg-gradient-to-r from-brand-500/10 via-indigo-500/5 to-brand-500/10 border-b border-brand-500/15 px-6 py-2 flex items-center justify-center gap-3">
            <Eye className="w-3.5 h-3.5 text-brand-600 animate-pulse" />
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-brand-800">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden sm:flex items-center gap-2 border-l border-slate-200 pl-4">
                <span className="text-[10px] text-slate-500">
                  ¡Hola, <span className="font-semibold text-slate-800">{clientName}</span>!
                </span>
              </div>

              <button
                onClick={onClose}
                className="w-8 h-8 rounded-lg bg-slate-50 border border-slate-200 flex items-center justify-center hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-slate-50">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-40 gap-4">
              <AlertTriangle className="w-10 h-10 text-red-500 opacity-40" />
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <h3 className="text-lg font-bold text-brand-700 mb-1">{data.projectName} - Lote {data.lotNumber}</h3>
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex-1 space-y-3">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-extrabold text-brand-600 tracking-tight">{data.acquisitionProgress}%</span>
              <span className="text-xs font-bold text-slate-400">{data.paidCuotas} de {data.totalCuotas} cuotas pagadas</span>
            </div>

            <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
              <div 
                className="bg-brand-600 h-full rounded-full transition-all duration-[1s]"
          ...
          <div className="bg-brand-50/50 border border-brand-100 rounded-2xl p-4 md:min-w-[280px] flex flex-col justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold text-brand-600 uppercase tracking-wider leading-none">
                {data.paidCuotas >= data.totalCuotas ? "Plan Completado" : `Próxima Cuota`}
              </p>
              {data.paidCuotas < data.totalCuotas && (
                <p className="text-xl font-extrabold text-brand-700 tracking-tight mt-1">
            ...
            {data.paidCuotas < data.totalCuotas && (
              <button
                ...
                className="flex items-center justify-center gap-1.5 w-full py-2.5 bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs rounded-xl transition-all shadow-sm active:scale-98 cursor-pointer"
              >
                          ...
                          <div className="flex items-center gap-2 justify-end sm:justify-start">
                            ...
                            {item.comprobanteCliente && (
                              <button
                                ...
                                className="text-brand-600 hover:text-brand-700 transition-colors p-1.5 rounded-lg hover:bg-emerald-50 inline-flex items-center justify-center cursor-pointer border border-emerald-100 bg-emerald-50/30"
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {/* Card 1: Contrato */}
          <div className="bg-slate-50/50 border border-slate-200 hover:border-brand-200 transition-all rounded-2xl p-5 flex flex-col items-center text-center">
            <div className="w-12 h-12 rounded-full bg-brand-50 flex items-center justify-center text-brand-600 mb-3 shadow-sm">
            ...
            {contratoDoc ? (
              <button
                ...
                className="text-xs font-bold text-brand-600 hover:text-brand-800 transition-colors mt-2 flex items-center gap-1 cursor-pointer"
              >
  ...
  return (
    <div className={hasMoraOrPenalty ? "w-full space-y-6 animate-fade-in" : "max-w-3xl mx-auto space-y-6 animate-fade-in"}>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center justify-between">
        ...
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${
          data.isUpToDate && !hasMoraOrPenalty ? "bg-emerald-50 text-emerald-700" : "bg-orange-50 text-orange-700"
        }`}>
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            ...
            <p className="text-xs text-slate-500 font-semibold mt-1">
              {data.clientName || "Cliente"} · {overdueCount > 0 ? (overdueCount === 1 ? "1 cuota vencida" : `${overdueCount} cuotas vencidas`) : "Cuotas al día"}{moraHistorica > 0 ? " · Mora pendiente" : ""}
            </p>
          </div>

          {/* Cuotas Selector Dropdown */}
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-2">
            <label htmlFor="cuotasSelectModal" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
              Selecciona cuántas cuotas pagar{overdueCount > 0 ? ` (mínimo ${overdueCount} vencida${overdueCount > 1 ? 's' : ''})` : ''}
            </label>
            <div className="relative">
              <select
                id="cuotasSelectModal"
                  ...
                  >
                    {receiptBase64 ? (
                      <div className="text-center">
                        <div className="w-12 h-12 rounded-full bg-emerald-50 flex items-center justify-center mx-auto mb-3 border border-emerald-200 text-emerald-700">
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm text-slate-800 font-bold focus:border-brand-600 focus:ring-2 focus:ring-brand-600/10 focus:outline-none cursor-pointer appearance-none animate-none"
                  style={{
