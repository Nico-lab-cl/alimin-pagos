...
import { formatCLP, formatDate, getDownloadFilename, downloadDocument, downloadCsv } from "@/lib/utils";
import { 
      ...
      <div className="bg-[#f7f9fb] sm:border border-[#E2E8F0] sm:rounded-3xl w-full max-w-[1200px] h-[100dvh] sm:h-[85vh] shadow-2xl animate-slide-up flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-white border-b border-[#E2E8F0] p-4 sm:p-6 flex flex-col md:flex-row items-start md:items-center justify-between shrink-0 gap-4">
          <div className="flex items-center gap-4 sm:gap-6 w-full md:w-auto">
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-brand-600/5 border border-brand-600/10 flex items-center justify-center relative overflow-hidden shrink-0">
              <User className="w-7 h-7 sm:w-8 sm:h-8 text-brand-600 relative z-10" />
              <div className="absolute inset-0 bg-brand-600/10 rounded-2xl animate-pulse"></div>
            </div>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] sm:text-sm font-semibold text-slate-400 uppercase tracking-[0.1em] mt-1.5 sm:mt-2 flex items-center gap-1.5 sm:gap-2 truncate">
                <Target className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-brand-600 shrink-0" />
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full md:w-auto overflow-x-auto pb-1 sm:pb-0 scrollbar-hide">
            <button
              onClick={async () => {
                ...
                await downloadCsv(csvContent, `Ficha_${selectedClient.clientName?.replace(/\s+/g, '_')}_${selectedClient.lotNumber}.csv`);
              }}
            ...
            {!isReadOnly && (
              <button
                ...
                className="px-3 sm:px-4 py-2 rounded-xl text-[9px] sm:text-[10px] font-bold uppercase tracking-widest border transition-all flex items-center gap-1.5 sm:gap-2 bg-brand-50 border-brand-200 text-brand-600 hover:bg-brand-100 whitespace-nowrap"
              >
        ...
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
          ...
          <div className="w-full md:w-24 border-b md:border-b-0 md:border-r border-[#E2E8F0] bg-white flex flex-row md:flex-col items-center justify-start md:justify-center lg:justify-start py-3 md:py-6 gap-2 md:gap-6 shrink-0 overflow-x-auto scrollbar-hide px-4 md:px-0">
            ...
            <button 
              ...
              className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center shrink-0 transition-all ${activeTab === "DOCUMENTS" ? "bg-brand-500/10 text-brand-700 border border-brand-500/20 shadow-sm" : "text-slate-400 hover:text-slate-700 hover:bg-slate-50"}`}
              title="Documentos"
            >
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-8 relative scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div className="space-y-1.5">
                        ...
                        <input type="text" value={editForm.name} onChange={e=>setEditForm({...editForm, name: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider">RUT Registrado</label>
                        <input type="text" value={editForm.rut} onChange={e=>setEditForm({...editForm, rut: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider">Correo / Usuario</label>
                        <input type="email" value={editForm.email} onChange={e=>setEditForm({...editForm, email: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider">Teléfono</label>
                        <input type="text" value={editForm.phone} onChange={e=>setEditForm({...editForm, phone: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div className="space-y-1.5">
                        ...
                        <input type="text" value={editForm.marital_status} onChange={e=>setEditForm({...editForm, marital_status: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider">Profesión u Oficio</label>
                        <input type="text" value={editForm.profession} onChange={e=>setEditForm({...editForm, profession: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider">Nacionalidad</label>
                        <input type="text" value={editForm.nationality} onChange={e=>setEditForm({...editForm, nationality: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider">Región</label>
                        <input type="text" value={editForm.address_region} onChange={e=>setEditForm({...editForm, address_region: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider">Comuna</label>
                        <input type="text" value={editForm.address_commune} onChange={e=>setEditForm({...editForm, address_commune: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider">Calle / Pasaje</label>
                        <input type="text" value={editForm.address_street} onChange={e=>setEditForm({...editForm, address_street: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider">Número</label>
                        <input type="text" value={editForm.address_number} onChange={e=>setEditForm({...editForm, address_number: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium" />
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <textarea 
                        ...
                        className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-950 focus:border-brand-600 focus:ring-1 focus:ring-brand-600/20 outline-none font-medium min-h-[80px] sm:min-h-[100px] resize-none"
                      />
                    </div>
                    ...
                    <div className="flex gap-2 sm:gap-3 pt-2">
                      ...
                      <button 
                        ...
                        disabled={isSaving}
                        className="flex-1 px-4 sm:px-6 py-3 rounded-xl bg-brand-600 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest hover:bg-brand-600/90 disabled:opacity-50 flex items-center justify-center gap-2 shadow-md"
                      >
                ...
                {isEditingFin ? (
                        ...
                        {finForm.installment_ranges.map((range: any, idx: number) => (
                          <div key={idx} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 bg-white p-3 rounded-xl border border-[#E2E8F0] shadow-sm">
                            ...
                            <div className="flex items-center gap-2 mt-2 sm:mt-0">
                              ...
                              <button onClick={() => { const newRanges = finForm.installment_ranges.filter((_: any, i: number) => i !== idx); setFinForm({...finForm, installment_ranges: newRanges})}} className="text-red-500 hover:text-red-600 w-10 h-10 flex items-center justify-center bg-red-50 rounded-lg"><Trash2 className="w-4 h-4"/></button>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <>
                    ...
                    <div className="bg-white p-4 sm:p-5 rounded-2xl border border-[#E2E8F0] flex flex-col justify-center shadow-sm">
                      ...
                      <p className="text-lg sm:text-xl font-bold text-emerald-700">{formatCLP(selectedClient.pie)}</p>
                    </div>
                    ...
                    <div className="bg-white p-4 sm:p-5 rounded-2xl border border-[#E2E8F0] flex flex-col justify-center shadow-sm">
                      ...
                      <p className="text-lg sm:text-xl font-bold text-emerald-700">{selectedClient.paidCuotas} Cuotas</p>
                    </div>
                    <div className="bg-white p-4 sm:p-5 rounded-2xl border border-[#E2E8F0] flex flex-col justify-center shadow-sm">
                    ...
                    <div className="col-span-1 sm:col-span-2 lg:col-span-3 mt-2 sm:mt-0">
                      <div className="h-3 sm:h-4 bg-slate-100 rounded-full overflow-hidden border border-[#E2E8F0] flex">
                        <div className="h-full bg-brand-600 transition-all" style={{ width: `${(selectedClient.paidCuotas / Math.max(selectedClient.totalCuotas, 1)) * 100}%` }}></div>
                      </div>
                      ...
                      <div className="border border-emerald-200 bg-emerald-50/[0.3] p-5 sm:p-6 rounded-3xl space-y-6 shadow-sm">
                        <div>
                          <h4 className="text-sm sm:text-base font-bold text-emerald-700 tracking-tight uppercase leading-none mb-1.5 flex items-center gap-2">
                            <DollarSign className="w-4 h-4 text-emerald-700" /> Registrar Pago Manual (Post-Venta)
                          </h4>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            ...
                            <div className="relative">
                              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-bold text-emerald-700">$</span>
                              <input
                                type="number"
                        ...
                        <div className="space-y-2">
                          <label className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider">
                            Comprobante de Pago <span className="text-emerald-700 font-bold">* Obligatorio</span>
                          </label>
                        ...
                        <button
                          ...
                          className="w-full py-3 rounded-xl bg-emerald-500 disabled:bg-emerald-500/20 disabled:text-white/20 text-white text-xs font-bold uppercase tracking-widest hover:bg-emerald-600 disabled:opacity-50 flex items-center justify-center gap-2 shadow-md transition-all mt-4"
                        >
            ...
            {activeTab === "MORA" && (
              <div className="max-w-4xl animate-fade-in mx-auto md:mx-0">
                <h3 className="text-lg sm:text-xl font-bold font-headline-sm text-red-600 tracking-tight uppercase leading-none mb-6 sm:mb-8">Cálculo de Multas</h3>
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    <div className="bg-red-50/30 border border-red-200 p-4 sm:p-6 rounded-2xl space-y-4 sm:space-y-6 shadow-sm">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                        ...
                        <button onClick={() => setFinForm({...finForm, mora_status: "AL_DIA"})} className={`flex-1 py-3 sm:py-4 px-4 rounded-xl text-[9px] sm:text-[10px] font-bold uppercase tracking-widest border transition-all ${finForm.mora_status === "AL_DIA" ? "bg-emerald-50 border-emerald-300 text-emerald-700 shadow-sm" : "bg-white border border-slate-200 text-slate-400 hover:bg-slate-50"}`}>
                          Al Día Sin Mora
                        </button>
                        <button onClick={() => setFinForm({...finForm, mora_status: "CONGELADO"})} className={`flex-1 py-3 sm:py-4 px-4 rounded-xl text-[9px] sm:text-[10px] font-bold uppercase tracking-widest border transition-all ${finForm.mora_status === "CONGELADO" ? "bg-slate-100 border-slate-400 text-slate-600 shadow-sm" : "bg-white border border-slate-200 text-slate-400 hover:bg-slate-50"}`}>
            ...
            {activeTab === "DOCUMENTS" && (
              <div className="max-w-4xl animate-fade-in mx-auto md:mx-0">
                ...
                {!isReadOnly && (
                  <div className="bg-brand-50/50 border border-brand-200 rounded-2xl p-4 sm:p-6 mb-6">
                    <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-brand-600 mb-3 sm:mb-4">Cargar Nuevo Documento</p>
                    <div className="flex flex-col sm:flex-row gap-3">
                      <input type="text" placeholder="Nombre del documento (ej. Carnet Identidad)" value={docName} onChange={e=>setDocName(e.target.value)} className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:border-brand-500 outline-none font-medium" />
                      <label className="px-6 py-3 rounded-xl bg-brand-600 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-wider hover:bg-brand-700 cursor-pointer flex items-center justify-center gap-2 transition-all shrink-0 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {loadingDocs ? (
                    <div className="col-span-full py-12 sm:py-16 flex flex-col items-center justify-center gap-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                      <Loader2 className="w-6 h-6 sm:w-8 sm:h-8 animate-spin text-brand-500/50"/>
                    </div>
                  ) : docs.length === 0 ? (
                    ...
                    <div key={doc.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl hover:border-brand-500/30 transition-all gap-3 sm:gap-0 shadow-sm">
                      <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto overflow-hidden">
                        <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center shrink-0"><FileText className="w-4 h-4 text-brand-500" /></div>
                        ...
                        {doc.hasFile === false ? (
                          ...
                          <>
                            ...
                            <button
                               ...
                               className="flex-1 sm:flex-none h-9 sm:w-8 sm:h-8 rounded-lg bg-slate-50 hover:bg-brand-50 hover:text-brand-600 flex items-center justify-center text-slate-500 transition-colors"
                               title="Descargar"
                             >
                      ...
                      {financialHistory.map(item => (
                        <div key={item.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-4 bg-white border border-slate-100 rounded-xl sm:rounded-2xl gap-2 sm:gap-0 hover:bg-slate-50 transition-colors shadow-sm">
                          <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
                            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 ${item.category === 'PIE' ? 'bg-amber-50 text-amber-700' : item.category === 'PENALTY' ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-700'}`}>
