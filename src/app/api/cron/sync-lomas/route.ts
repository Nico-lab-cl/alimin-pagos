...
//
// Toda corrida (exitosa, rechazada por credencial o caida por error) queda
// registrada en AuditLog con entity "SyncLomas", visible en /admin/audit bajo
// el filtro "Sincronizacion Lomas". Sin ese registro no hay forma de saber si
// el cron esta corriendo o no.
...
// Deja constancia de la corrida. Nunca debe romper la sincronizacion: si el
// log falla, se avisa por consola y el endpoint sigue su curso normal.
async function logRun(action: "CREATE" | "OTHER", details: string) {
  try {
    await prisma.auditLog.create({
      data: { action, entity: "SyncLomas", entity_id: PROJECT_SLUG, details },
    });
  } catch (e) {
    console.error("[sync-lomas] no se pudo escribir el registro de auditoria:", e);
  }
}
...
async function handle(req: NextRequest) {
  ...
  if (!secret || (provided !== `Bearer ${secret}` && provided !== secret)) {
    // Se registra el rechazo porque es el modo de falla mas probable del cron:
    // si CRON_SECRET no esta configurada en el servidor, TODAS las llamadas
    // caen aca y el puente nunca corre, sin dejar ninguna otra senal.
    await logRun("OTHER", secret
      ? "Corrida RECHAZADA (401): la credencial enviada no coincide con CRON_SECRET."
      : "Corrida RECHAZADA (401): CRON_SECRET no esta configurada en el servidor, por lo que el puente Lomas->Portal no puede ejecutarse.");
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }
  const dryRun = req.nextUrl.searchParams.get("dryRun") === "true";
  const prefijo = dryRun ? "[SIMULACION, no se escribio nada] " : "";
  ...
  try {
    ...
    if (!project) {
      await logRun("OTHER", `${prefijo}Corrida FALLIDA: el proyecto "${PROJECT_SLUG}" no existe en el portal.`);
      return NextResponse.json({ error: "Proyecto lomas-del-mar no existe" }, { status: 404 });
    }
    ...
    const resumen = `${prefijo}Sincronizacion Lomas->Portal OK en ${elapsed()}. Candidatas: ${finalistas.length}. Nuevos: ${nuevos}. Ya existian: ${omitidos}. Conflictos omitidos: ${conflictos.length}. Comprobantes copiados: ${receiptsCreados}. Documentos copiados: ${docsCreados}.` +
      (detalle.length ? ` Ingresados: ${detalle.join("; ")}.` : "") +
      (conflictos.length ? ` Conflictos: ${conflictos.map((c) => c.join(" vs ")).join(" | ")}.` : "");
    await logRun(nuevos > 0 && !dryRun ? "CREATE" : "OTHER", resumen);
    ...
    // Se registra lo alcanzado antes de la caida: sin esto, un error dejaba la
    // corrida a medias sin ningun rastro de cuanto habia entrado.
    await logRun("OTHER", `${prefijo}Corrida CAIDA tras ${elapsed()} con error: ${e?.message || String(e)}. Alcanzo a ingresar ${nuevos} cliente(s)${detalle.length ? ` (${detalle.join("; ")})` : ""}, ${receiptsCreados} comprobante(s) y ${docsCreados} documento(s).`);
    console.error("[sync-lomas] corrida fallida:", e);
    return NextResponse.json({ error: "Error en la sincronizacion", detalle: e?.message || String(e) }, { status: 500 });
  }
}
